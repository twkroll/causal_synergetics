...
The five-stage vertical slice is accepted as coherent feasibility evidence without novelty promotion. The next unresolved capability was held-out intervention prediction from compact response/plasticity representations.
...
The frozen benchmark used 81 exact function-equivalent states, a fixed 2D response-PCA coordinate, fixed held-out interventions, B0/B1/B2 baselines, ceilings/nulls and disjoint thresholds; no second coordinate/family was permitted.
...
Frozen facts: candidate held-out `R2_state=1.0`; B2 equal-dimensional raw PCA `R2_state=0.999883026432542`; candidate-minus-B2 `0.000116973567458323 < 0.05`; all sanity/leakage/oracle/regression checks passed; no retuning.
...
Canonical prompt: `research/master/prompts/master_neural_response_coordinate_weak_integration_gate_0_1.md`.
...
The sole unresolved prerequisite is a neutral nuisance-/reparameterisation-invariance discriminator; direct broader scaling and arbitrary second-family repair were rejected.

### DEC-028 — Nuisance-invariance discriminator selected as sole revision prerequisite
Status: FROZEN

Any next nuisance construction must derive from explicit model reparameterisation/gauge/symmetry or another independently justified redundancy, not anticipated baseline failure. If no such construction can be specified, park the coordinate direction.

### DEC-029 — Neural Response Coordinate Nuisance-Invariance Specification Gate 0.1 authorised
Status: SATISFIED / CLOSED

Canonical prompt: `research/master/prompts/master_neural_response_coordinate_nuisance_invariance_specification_gate_0_1.md`.
...
The single frozen nuisance subgroup is `Q(phi)=R(phi)⊕I3`, sampled at eight equally spaced angles `phi_j=j*pi/4`. No alternative gauge group, rotation plane, amplitude or nuisance family is authorised.
